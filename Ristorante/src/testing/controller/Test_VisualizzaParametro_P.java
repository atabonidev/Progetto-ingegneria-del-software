...
import main_package.controller.handlers.VisualizzaParametro;
...
public class Test_VisualizzaParametro_P {
    ...
    @BeforeEach
    public void setUp(){
        File file = new File("./src/testing/input_cases/case_VisualizzaParametro_P.txt");
        try {
    ...
    void VisualizzaParametro(){
        new VisualizzaParametro(new ServicesFactory("jdbc:sqlite:RistoranteTest.db", "src/testing/filesTest/parametriConfigurazioneTEST.json", "src/testing/filesTest/parametriRistoranteTEST.json").getGestoreRepository()).execute(session, cliView);

        String expectedOutput = "--- VISUALIZZAZIONE PARAMETRO ----\n" +
                "0\tEsci\n" +
                "1\tCarico lavoro per persona\n" +
                "2\tNumero posti a sedere\n" +
                "3\tInsieme delle bevande\n" +
                "4\tInsieme dei generi extra\n" +
                "5\tCorrispondenza piatti - ricette\n" +
                "6\tRicette\n" +
                "7\tMenu tematici\n" +
                "--------------------------------\n\n"+
